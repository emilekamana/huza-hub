...
exports.signinServiceProvider = async (req, res, next) => {
    try {
        ...
        if (!username || !password) {
            return next(new ErrorResponse("Please provide both username and password", 403));
        }
...
exports.allServiceProviders = async (req, res, next) => {
    ...
    if (req.query.location) {
        locationFilter.location = req.query.location;
    }

    try {
        const count = await ServiceProvider.countDocuments({ ...keyword, ...locationFilter });
        const serviceProviders = await ServiceProvider
            .find({ ...keyword, ...locationFilter })
            .sort({ createdAt: -1 })
            .select('-password')
